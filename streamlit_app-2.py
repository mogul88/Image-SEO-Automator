...
def safe_error_text(error, limit=900):
...
with st.sidebar:
    ...
    openrouter_model = st.text_input(
        ...
        placeholder="Leave blank for configured backup model",
    )
...
with right:
    ...
    st.info(
        ...
        "If Gemini fails, configured OpenRouter backup will run."
    )
...
if start:
    ...
    if not dry_run and not (
        wp_site_url.strip()
        and wp_username.strip()
        and wp_app_password.strip()
    ):
        st.error(
            "Live Upload ke liye WordPress URL, username aur "
            "Application Password required hain."
        )
        st.stop()
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            if len(manual_topics) >= len(image_paths):
                ...
                st.error(f"AI failed: {safe_error_text(error)}")
                st.info(
        ...
        for index, image_path in enumerate(image_paths):
            ...
            status.info(
                f"Image {index + 1}/{len(image_paths)} — Compressing..."
            )
            ...
            if not dry_run:
                status.info(
                    f"Image {index + 1}/{len(image_paths)} — "
                ...
                try:
                    ...
                    if wp_url:
                        ...
                        log(
                            f"Uploaded but URL missing: {output_name}"
                        )

                except WordPressUploadError as error:
                    upload_error = safe_error_text(error)
                    ...
                    log(
                        f"WordPress FAILED: {output_name}\n"
                        f"Reason: {upload_error}"
                    )
                    st.error(
                        f"WordPress upload failed for {output_name}: "
                        f"{upload_error}"
                    )
                    ...
                    st.error(
                        f"Unexpected WordPress error for {output_name}: "
                        f"{upload_error}"
                    )
        ...
        if upload_failures:
            st.warning(
                ...
                f"{len(uploaded_urls)} WordPress upload(s) succeeded, "
                f"{len(upload_failures)} failed. "
                "ZIP package phir bhi ready hai."
            )
        else:
        ...
        if uploaded_urls:
            ...
            st.success(
                f"✅ {len(uploaded_urls)} uploaded image URL(s) ready."
            )
            ...
            components.html(
                ...
                <script>
                    ...
                    const button = document.getElementById("copyAllUrls");
                    ...
                    button.addEventListener("click", async () => {{
                        ...
                        if (copied) {{
                            ...
                            status.innerText = "URLs clipboard mein copy ho gaye.";
                        }} else {{
                            button.innerText = "Use Copy Icon Above";
                            status.innerText = "Browser ne direct copy block ki. Code box ka copy icon use karein.";
                        }}
        ...
        if upload_failures:
            st.markdown("## ⚠️ WordPress Upload Failures")
            st.dataframe(
                upload_failures,
                use_container_width=True,
            )
